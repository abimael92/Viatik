...
/**
 * Apple HealthKit provider backed by Capacitor's native health plugin.
 * Raw HealthKit records are reduced to a validated daily aggregate before they
 * cross the provider boundary.
 */
export class HealthKitAdapter implements HealthProvider {
  ...
  async requestPermissions(): Promise<HealthPermissionResult> {
    try {
      ...
      return { platform: "ios", state: "authorized", readSteps: true };
    } catch {
      return { platform: "ios", state: "denied", readSteps: false };
    }
  }
...
function sumStepResults(encodedResults?: string): number {
  ...
  try {
    ...
    throw new Error("HealthKit returned an invalid steps response.");
  }
