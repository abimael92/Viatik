...
- **Application version/commit:** `main` at pre-fix HEAD (`59a403d`)
...
2. **Implementation:** migration
   `00000000000037_fix_profiles_rls.sql` (see above layers).
...
5. **Verification:** unit security tests, `pnpm typecheck`, `pnpm lint`.
   Migration applied directly to the linked hosted Supabase project and
   verified against `pg_policies`/`pg_proc`/`information_schema.views`.
...
- **Fix commit/PR:** migration `00000000000037_fix_profiles_rls.sql`, applied
  directly to the hosted project; repository/test changes committed to `main`.
- **Verification evidence:**
  - `pnpm test supabase/profiles-rls-security.test.ts` — PASS.
  - `pnpm typecheck` — PASS.
  - `pnpm lint` — PASS.
  - Applied to the linked hosted Supabase project; confirmed via
    `pg_policies` (only `profiles_select_self` remains for SELECT), `pg_proc`
    (`get_profile_public_data` exists), and `information_schema.views`
    (`profile_public_data` exists).
- **Bug ledger entry:** [`specs/bug-ledger.md`](./bug-ledger.md) — "P0:
  `profiles` RLS exposed PII…".
