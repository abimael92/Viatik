...
| 2026-09-17 | Vote summary avatars fell back to user IDs instead of the account avatar configured by each voter. | **Invariant:** Every rendered voter avatar uses the account's uploaded avatar or avatar seed when public profile data is available, with a deterministic fallback only when it is unavailable. | `features/activities/components/proposals-section.tsx`; `activity-vote-card.test.tsx`; implementation verified, commit pending. |
...
| 2026-09-21 | Trip Feed rendered a fallback avatar instead of the signed-in user's configured avatar. | **Invariant:** Every signed-in user's feed avatar uses the live local profile avatar URL or seed; collaborator avatars use authorized public profile data, with a deterministic fallback only when unavailable. | `.ai/specs/bug-feed-avatar.md`; `features/feed/components/shared-trip-feed.test.tsx`; implementation verified, commit pending. |
| 2026-09-21 | Expense-share sync could replay a share before its parent expense and repeatedly fail with `Expense does not exist`. | **Invariant:** Sync replays expense parents before expense shares; a missing remote parent requeues the local expense and resets the dependent share for a later retry. | `.ai/specs/bug-expense-share-sync-order.md`; `lib/sync/sync-engine.test.ts`; implementation verified, commit pending. |
| 2026-09-21 | A legacy expense payer such as `dude` caused `sync_cas_upsert` to fail with an invalid UUID cast. | **Invariant:** Expense and expense-share identities must be canonical UUIDs or saved-traveler UUID keys before crossing the RPC boundary; unresolved legacy strings are rejected locally. | `.ai/specs/bug-invalid-expense-identity-sync.md`; `lib/sync/sync-engine.test.ts`; implementation pending verification. |
| 2026-09-21 | The People tab omitted Viatik trip members unless they also had a named traveler/contact row. | **Invariant:** People displays every trip member and named traveler exactly once; linked members are deduplicated against their traveler row, and member identity uses authorized public profile data. | `.ai/specs/bug-people-tab-members.md`; `features/collaboration/components/people-panel.test.tsx`; implementation verified, commit pending. |
