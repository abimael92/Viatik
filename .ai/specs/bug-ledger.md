...
|| 2026-09-22 | Finance mapper round-trips fabricated nullable traveler identity fields, and expense repository tests used display-name-shaped IDs that violated the UUID sync boundary. | **Invariant:** Mappers preserve optional traveler identity fields without inventing null-valued properties; expense participants crossing the sync boundary are UUID-backed profile or saved-traveler identities. | `.ai/specs/bug-invalid-expense-identity-sync.md`; `lib/supabase/mappers.ts`; `lib/db/transactional-writes.test.ts`; `features/feed/data/dexie-feed-repository.test.ts`; verification passed, commit pending. |
|| 2026-09-22 | Offline activity personal budget inserts failed in the CAS RPC because `created_at` and `updated_at` were populated as explicit nulls. | **Invariant:** Every server-side CAS insert supplies non-null metadata timestamps explicitly; client payload timestamps cannot be relied on to activate table defaults after composite JSON population. | `.ai/specs/bug-personal-budget-cas-timestamps.md`; `supabase/migrations/00000000000057_fix_activity_personal_budget_cas_timestamps.sql`; `supabase/sync-schema.test.ts`; migration deployed and local/browser verification passed, affected-session retry pending. |
| 2026-09-23 | Saved activity sub-tasks disappeared when the editor reopened and were absent from Home progress. | The linked Supabase project stopped at migration 57, so migrations 58–60 for the checklist column, CAS sync, validation, and authorization were absent. Stale client snapshots and hidden Home write failures compounded the issue. **Invariant:** Features backed by additive schema changes are not complete until linked migration parity is verified; successful local writes immediately replace stale snapshots, while optimistic Home progress rolls back visibly on failure. | Migrations 58–60 deployed and parity verified; `.ai/specs/bug-home-checklist-persistence-feedback.md`; regression tests verified, commit pending. |
| 2026-09-23 | Home displayed “Nothing scheduled today” while valid itinerary activities existed. | The participation filter interpreted a missing current-user roster entry as non-attendance, and the active-trip builder discarded all earlier trip days. **Invariant:** Home hides an activity only for an explicit current-user `declined` or `pending` entry; roster absence is not a decline, and active timelines retain past/current/future itinerary days for windowing. | `.ai/specs/bug-home-timeline-missing-roster.md`; participation and timeline regression tests verified, commit pending. |
| 2026-09-24 | Landing navigation hydrated in Spanish while the server HTML was English, so React discarded the server tree. | The i18n provider read `localStorage` and `documentElement.lang` inside the first client render. **Invariant:** The first i18n render always uses the server default locale (`en`); a stored locale is applied only after hydration. | `lib/i18n/i18n-provider.tsx`; `lib/i18n/i18n-provider.test.tsx`. |
| 2026-09-24 | Root layout logged “Encountered a script tag while rendering React component” for the theme/language init script. | React 19 warns and does not execute a `<script>` created during client render, including `next/script`. **Invariant:** The blocking init script is emitted only in the server HTML; the client omits it and reapplies the stored theme and language before paint. | `components/theme-initializer.tsx`; `app/layout.tsx`; browser check confirmed the warning is gone and `data-theme` is applied. |
| 2026-09-24 | Large parts of the app stayed in English after switching the language to Spanish. | User-visible copy was hardcoded in components instead of going through `translate`. **Invariant:** Frontend copy shown to travelers is keyed in `lib/i18n` for both `en` and `es`; new screens must not ship literal English in the render path. | `lib/i18n/frontend-copy.ts`; `lib/i18n/i18n-provider.test.tsx`. |
| 2026-09-24 | Share-level `settleShare` could mark an `ExpenseShare.settledAt` while the trip also stored an immutable `ExpenseSettlement`, creating two repayment truths. | **Invariant:** Pairwise repayment is recorded only by inserting an `ExpenseSettlement`. Expense and share rows stay unchanged; `ExpenseShare.settledAt` is not a settlement write path. | `features/expenses/data/dexie-expense-repository.ts`; `lib/sync/cloud-sync.ts`; `features/expenses/components/settle-up-sheet.test.tsx`; verification pending. |
| 2026-09-25 | Viewers could not mark attendance because it lived in the activity editor, and a direct member add did not notify the added person. | **Invariant:** Any trip member may change only their own activity attendance from the card or read-only details, and that write is queued on the activity. A new direct member insert queues a `trip_added` notification whose payload is the trip name. | `.ai/specs/bug-viewer-rsvp-and-trip-added.md`; migration `00000000000065_member_rsvp_and_trip_added.sql`; unit tests passed. Hosted database still needs migration 65. |
| 2026-09-25 | Home stayed empty and showed the offline banner while the account still owned its trips. | **Invariant:** A browser report of offline does not skip the trip download. The offline banner is shown only after a sync request fails because the network is unreachable. Trips already fetched are kept when a later table fails. | `.ai/specs/bug-offline-banner-hides-trips.md`; `lib/sync/cloud-sync.test.ts`; published site still needs this build. |
