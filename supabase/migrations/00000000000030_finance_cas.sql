-- Phase 2A: register the new finance entities and columns in the atomic
-- compare-and-swap sync functions. Both functions are recreated in full so a
-- single migration can extend the existing (migration 16) implementations.
...
begin
  ...
  v_entity := case p_entity
    ...
    when 'userWallet' then 'user_wallets' when 'user_wallets' then 'user_wallets'
    when 'dailyBudgetOverride' then 'daily_budget_overrides' when 'daily_budget_overrides' then 'daily_budget_overrides'
    else null
  end;
  ...
  elsif v_entity = 'user_wallets' then select updated_at,to_jsonb(t) into v_current_updated_at,v_current from public.user_wallets t where id=p_id for update;
  elsif v_entity = 'daily_budget_overrides' then select updated_at,to_jsonb(t) into v_current_updated_at,v_current from public.daily_budget_overrides t where id=p_id for update;
  end if;
  ...
  elsif v_entity = 'user_wallets' then delete from public.user_wallets where id=p_id and updated_at=p_base_updated_at;
  elsif v_entity = 'daily_budget_overrides' then delete from public.daily_budget_overrides where id=p_id and updated_at=p_base_updated_at;
  end if;
