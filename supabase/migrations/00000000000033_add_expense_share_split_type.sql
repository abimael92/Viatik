-- Add the missing `split_type` column to `expense_shares` and fix trip
-- coordinate syncing in the CAS upsert.
--
-- Two synchronization defects are corrected here:
--
-- 1. The local domain model (Dexie schema v18) emits a `splitType` for every
--    expense share, and the `sync_cas_upsert` RPC has always referenced
--    `split_type` in its UPDATE branch. The column was never added to the
--    table, so any expense-share upsert replayed from the outbox failed with
--    "column does not exist", got marked failed, and was dropped after the
--    retry limit. We add the column, backfill existing shares from their
--    parent expense, and restore the not-null guarantee.
--
-- 2. The `trips` UPDATE branch omitted `latitude`, `longitude`, `place_id`,
--    and `time_zone`, so editing a trip's destination never synchronized its
--    new coordinates to collaborators (breaking weather and the offline map
--    for other devices). The INSERT path already mapped them via
--    jsonb_populate_record; the UPDATE branch is recreated to include them.

-- Expense shares: add the missing split methodology column.
alter table public.expense_shares
  add column split_type public.expense_split_type;

-- Backfill existing shares from their parent expense's split type.
update public.expense_shares es
set split_type = e.split_type
from public.expenses e
where es.expense_id = e.id;

-- Any share left without a parent (edge case) falls back to 'equal'.
...
-- Recreate the CAS upsert so trip coordinate edits synchronize. The
-- expense_shares branch already references `split_type` and now has the
-- column to back it.
