-- Trip lifecycle status in the CAS sync RPC.
--
-- `sync_cas_upsert` updated the `trips` row with an explicit column list that
-- omitted `status`, `started_at`, and `completed_at`. New trips stored status
-- (via jsonb_populate_record on insert), but every UPDATE silently dropped the
-- lifecycle columns, so cancelling/ending a trip never persisted to the remote
-- and the pull/realtime layer kept reverting it to `planned` locally.
--
-- Recreate the RPC with the lifecycle columns included in the trips UPDATE.
