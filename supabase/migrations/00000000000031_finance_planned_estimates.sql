-- Phase 2B: planned budget estimates on itinerary activities.
--
-- `activities.estimated_cost` carries the planned/estimated cost of an
-- itinerary item in the trip's base currency (minor units, numeric(12,2)).
-- Used by the finance dashboard's "planned" liability so `True Leftover`
-- subtracts both actuals and upcoming estimates from a member's wallet.
--
-- The CAS upsert is recreated to include the new column in the activity UPDATE
-- branch; the INSERT path already maps all payload keys via jsonb_populate_record.

alter table public.activities add column estimated_cost numeric(12, 2);
...
begin
  ...
  if v_entity = 'trips' then
    ...
    if not found then
      ...
      update public.activities t set trip_id=(p.r).trip_id, day_date=(p.r).day_date, title=(p.r).title, description=(p.r).description, location=(p.r).location, category=(p.r).category, start_time=(p.r).start_time, end_time=(p.r).end_time, position=(p.r).position, estimated_cost=(p.r).estimated_cost, created_by=(p.r).created_by, deleted_at=(p.r).deleted_at from p where t.id=v_id and t.updated_at=p_base_updated_at returning to_jsonb(t.*) into v_applied;
    end if;
