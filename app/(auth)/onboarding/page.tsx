...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ next?: string; setup?: string }> }) {
  ...
  const initialName = profile?.full_name?.trim() || String(authData.user.user_metadata?.full_name ?? "").trim();
  ...
  return (
    <main className="grid min-h-dvh place-items-center px-4 py-12">
      <section className="w-full max-w-md rounded-2xl border bg-card p-6 shadow-sm sm:p-8" aria-labelledby="onboarding-title">
        ...
        <OnboardingForm email={email} next={destination} initialName={initialName} />
      </section>
    </main>
  );
}
